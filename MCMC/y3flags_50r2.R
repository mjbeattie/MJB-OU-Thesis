...
save( codaSamples , file=paste0("y3flags_50r2","coda.Rdata") )

s1 <- summary(codaSamples)
s1

#dicSamples <- extract(runJagsOut, what='dic') 
#save( dicSamples, file=paste0("y3flags_50r2","DIC.Rdata"))

library(ggmcmc)
s = ggs(codaSamples)
save (s, file=paste0("y3flags_50r2", "ggs.Rdata"))

#d = ggs_density(s)

#c = ggs_crosscorrelation(s)

#fileNameRoot = "y3flags_50r2"
