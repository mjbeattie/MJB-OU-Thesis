...
newdf$polyabuse <- newdf$cocflag + newdf$pcpflag + newdf$ecsflag + newdf$oxyflag + 
                   newdf$cpnmthfg
...
newdf$minafu <- pmin(newdf$ircocage, newdf$irpcpage, newdf$irecsage, newdf$iroxyage, 
                     newdf$irmthage)

# Create a maximum frequency variable
newdf$maxfy <- pmax(newdf$ircocfy, newdf$irhalfy, newdf$iroxyfy, newdf$irmthfy)
...
save( codaSamples , file=paste0("y1poly_50r3","coda.Rdata") )

s1 <- summary(codaSamples)
s1

#dicSamples <- extract(runJagsOut, what='dic') 
#save( dicSamples, file=paste0("y1poly_50r3","DIC.Rdata"))

library(ggmcmc)
s = ggs(codaSamples)
save (s, file=paste0("y1poly_50r3", "ggs.Rdata"))
