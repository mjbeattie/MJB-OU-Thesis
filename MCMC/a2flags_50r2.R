...
# Run the chains:
jagsModel = jags.model( file="TEMPmodel2.txt" , 
                        data=dataList , 
                        inits=initsList , 
                        n.chains=3 , 
                        n.adapt=500 )
...
save( codaSamples , file=paste0("a2flags_50r2","coda.Rdata") )

s1 <- summary(codaSamples)
s1

#dicSamples <- extract(runJagsOut, what='dic') 
#save( dicSamples, file=paste0("a2flags_50r2","DIC.Rdata"))

library(ggmcmc)
s = ggs(codaSamples)
save (s, file=paste0("a2flags_50r2", "ggs.Rdata"))

#d = ggs_density(s)

#c = ggs_crosscorrelation(s)

#fileNameRoot = "a2flags_50r2"
