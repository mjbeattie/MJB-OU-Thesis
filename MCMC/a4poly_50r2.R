...
newdf$minafu <- pmin(newdf$ircocage, newdf$ircrkage, newdf$irpcpage, newdf$irhalage,
                     newdf$irlsdage, newdf$irecsage, newdf$iroxyage, newdf$irmthage,
                     newdf$iranlage)
...
save( codaSamples , file=paste0("a4poly_50r2","coda.Rdata") )

s1 <- summary(codaSamples)
s1

#dicSamples <- extract(runJagsOut, what='dic') 
#save( dicSamples, file=paste0("a4poly_50r2","DIC.Rdata"))

library(ggmcmc)
s = ggs(codaSamples)
save (s, file=paste0("a4poly_50r2", "ggs.Rdata"))
