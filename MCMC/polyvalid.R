...
newdf$polyabuse <- newdf$cocflag + newdf$crkflag + newdf$DILAUD2 + newdf$METHDON2 +
                   ...
                   newdf$MORPHIN2 + newdf$onlyoxycod + newdf$ecsflag + newdf$oxyflag +
                   newdf$cpnmthfg + newdf$CODEINE2
...
g = g + geom_point(position = position_jitter(height = 0.015, width =0)) +
    ggtitle("Actual Use vs Probability: Adult 2012-2014") + theme_bw()
g
...
g <- ggplot(data=df, aes(x=fpr,y=tpr)) + ylab("True Positive Rate") + xlab("False Positive Rate") +
     ggtitle("ROC for Revised Adult Model 2012-2014") + theme_bw()
...
usemean <- paste("Mean =", round(mean(smalldf[smalldf$herflag==1,]$mu),4))
usemed <- paste("Median =", round(median(smalldf[smalldf$herflag==1,]$mu),4))
nonusemean <- paste("Mean =", round(mean(smalldf[smalldf$herflag==0,]$mu),4))
nonusemed <- paste("Median =", round(median(smalldf[smalldf$herflag==0,]$mu),4))


df2 <- smalldf[smalldf$herflag==1,]
df3 <- smalldf[smalldf$herflag==0,]
g2 <- ggplot(data=df2, aes(x=mu)) + geom_density(color='red') + theme_bw() + 
      ggtitle("Distribution of mu for Heroin Users, Adult 2012-2014") +
      theme(plot.title=element_text(size=10)) + geom_text(x=.5, y=1.5, label=usemean) +
      geom_text(x=.5, y=1.25, label=usemed)
g2

g3 <- ggplot(data=df3, aes(x=mu)) + geom_density(color='blue') + theme_bw() +
      ggtitle("Distribution of mu for Heroin Non-Users, Adult 2012-2014") +
      theme(plot.title=element_text(size=10)) + geom_text(x=.5, y=400, label=nonusemean) +
      geom_text(x=.5, y=350, label=nonusemed)
g3
