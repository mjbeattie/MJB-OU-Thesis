...
# 4) Examines polyabuse, treatment, perceived risk, access, 
#    needle use
...
newdf$polyabuse <- newdf$cocflag + newdf$crkflag + newdf$pcpflag + newdf$ecsflag + 
                   newdf$lsdflag + newdf$otherpain
...
newdf$minafu <- pmin(newdf$ircocage, newdf$ircrkage, newdf$irecsage, newdf$irlsdage,
                     newdf$iranlage, newdf$irpcpage)

# Create a maximum frequency variable
newdf$maxfy <- pmax(newdf$ircocfy, newdf$ircrkfy, newdf$irhalfy, newdf$iranlfy)
...
writeLines( modelString , con="TEMPmodelP2.txt" )

initsList = list(zeta0=0, zeta1=0, alpha2=0, alpha4=0, zeta5=0, alpha6=0)

# Run the chains:
jagsModel = jags.model( file="TEMPmodelP2.txt" , 
                        data=dataList , 
                        inits=initsList , 
                        n.chains=3 , 
                        n.adapt=500 )
...
save( codaSamples , file=paste0("y2poly_50r3","coda.Rdata") )

s1 <- summary(codaSamples)
s1

#dicSamples <- extract(runJagsOut, what='dic') 
#save( dicSamples, file=paste0("y2poly_50r3","DIC.Rdata"))

library(ggmcmc)
s = ggs(codaSamples)
save (s, file=paste0("y2poly_50r3", "ggs.Rdata"))
