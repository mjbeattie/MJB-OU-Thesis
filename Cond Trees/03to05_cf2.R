...
fit.df03to05ADLT <- cforest(herflag ~ ., data = df03to05ADLT.train, 
                            controls = cforest_unbiased(mtry=5, ntree=300))
endtime <- Sys.time()
...
saveRDS(fit.df03to05ADLT, "03to05ADLT_cf2.rds")         # Save fit dataset
write.csv(varimps, file ="03to05ADLT_cf2_imp.csv", 
...
saveRDS(pred, "cf2_test_pred.rds")
