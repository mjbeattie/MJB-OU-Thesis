...
smp_size <- floor(0.25 * nrow(df03to05ADLT))
set.seed(123)
...
fit.df03to05ADLT <- cforest(herflag ~ ., data = df03to05ADLT.train, 
                            controls = cforest_unbiased(mtry=5, ntree=200))
endtime <- Sys.time()
endtime - starttime
...
saveRDS(fit.df03to05ADLT, "df03to05ADLT_cf1.rds")       # Save fit dataset
write.csv(varimps, file ="03to05ADLT_cf1_imp.csv", 
...
pred <- as.data.frame(Predict(fit.df03to05ADLT, newdata=df03to05ADLT.test))
colnames(pred)[1] <- "fitted.values"
pred$prediction <- ifelse(pred$fitted.values>=0.2,1,0)
confusionMatrix(as.factor(pred$prediction), as.factor(df03to05ADLT.test$herflag))
